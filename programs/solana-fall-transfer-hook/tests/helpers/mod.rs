...
pub fn setup() -> (LiteSVM, Keypair, Address) {
    ...
    // Challenge 4: load the mover program next to the hook
    svm.add_program(
        token_mover::id(),
...
// Challenge 4 - move tokens through token-mover; hook accounts go in remaining accounts
pub fn build_program_transfer_ix(
    ...
    // Same three pushes as build_transfer_with_hook_ix. Hook program FIRST, rate limit WRITABLE.
    ix.accounts
